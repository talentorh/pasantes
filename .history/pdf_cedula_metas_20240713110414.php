...
/* 
$sql = "SELECT * FROM descripcionesmetas2023 WHERE id_empleado = :id_empleado";
$resultado = $conexion->consulta($sql);

$metas = [];
if ($resultado->num_rows > 0) {
    while($row = $resultado->fetch_assoc()) {
        $metas[] = $row['descripcion'];
    }
} else {
    echo "No se encontraron metas para el empleado con ID $empleado_id";
    exit;
}
